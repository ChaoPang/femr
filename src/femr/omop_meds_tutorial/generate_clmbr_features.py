...
def get_clmbr_features_name(label_name: str, observation_window: Optional[int] = None) -> str:
    if observation_window:
        return label_name + '_clmbr_' + str(observation_window)
    return label_name + '_clmbr'
...
def main():
    ...
    with meds_reader.SubjectDatabase(args.meds_reader, num_threads=6) as database:
        ...
        for label_name in labels:
            clmbr_features_name = get_clmbr_features_name(label_name, args.observation_window)
            feature_output_path = features_path / f"{clmbr_features_name}.pkl"
            training_metrics_file = flops_path / f"{clmbr_features_name}.json"
            ...
            features = femr.models.transformer.compute_features(
                db=database,
                model_path=str(pretraining_data / "clmbr_model"),
                labels=typed_labels,
