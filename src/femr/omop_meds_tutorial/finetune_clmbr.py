...
from .generate_clmbr_features import get_clmbr_features_name
    ...
    with meds_reader.SubjectDatabase(args.meds_reader, num_threads=6) as database:
        for label_name in labels:
            if args.observation_window:
                label_output_dir = output_dir / label_name / f"clmbr_{args.observation_window}"
            else:
                label_output_dir = output_dir / label_name / f"clmbr"
            ...
            with open(pretraining_data / 'features' / f"{clmbr_features_name}.pkl", 'rb') as f:
